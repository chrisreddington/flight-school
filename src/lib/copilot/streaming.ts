 ...
 *
 * Resolves the profile honouring any capabilities the conversation has
 * already attached so the cache fingerprint stays monotonic across
 * turns (fixes the multi-turn drift CRITICAL — see
 * `conversation-capabilities.ts`).
 *
 * @param identity - per-request `{ userId, gitHubToken }` from `requireUserContext`
 * @param prompt - the user's message
 * @param options - profile + capability + conversation overrides
 */
export async function createChatStreamingSession(
  ...
  const { profile, capabilities, operationName, conversationId } = options;
