...
export function SelfExplanationCard({ challengeId, dateKey, onSkip, onSave }: SelfExplanationCardProps) {
  const [text, setText] = useState('');
  const trimmedText = text.trim();

  return (
    <div className={styles.card} data-challenge-id={challengeId} data-date-key={dateKey}>
      <h4 id="self-explanation-label" className={styles.title}>
        What did you learn?
      </h4>
      ...
      <Stack direction="horizontal" gap="condensed" justify="end">
        <Button variant="primary" size="small" onClick={() => onSave(trimmedText)} disabled={trimmedText.length === 0}>
          Save & Continue
        </Button>
