  ...
  if (isLoading) {
    return (
      <div className={styles.guidedPanel}>
        <div className={styles.guidedPanelLoading}>
          <Spinner size="small" /> Building your guided plan...
        </div>
      </div>
    );
  }
  ...
  return (
    <section className={styles.guidedPanel} aria-live="polite">
      <div className={styles.guidedPanelHeader}>
        <span className={styles.guidedStepText}>
          Step {currentStep + 1} of {plan.totalSteps}
        </span>
        <div className={styles.guidedHeaderActions}>
      ...
      <div className={styles.guidedReflectWrapper}>
        <Button size="small" variant="default" onClick={() => setShowElaboration((v) => !v)}>
          💭 Reflect: Why does this work?
        </Button>
      </div>
      ...
      <div className={styles.guidedNav}>
        ...
        {isLast ? (
          ...
          <Button
            ...
            variant="primary"
            onClick={() => {
              setCurrentStep((v) => Math.min(plan.steps.length - 1, v + 1));
              setShowElaboration(false);
            }}
          >
            Next Step →
